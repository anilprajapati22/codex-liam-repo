    ...
    try:
        ...
        if cache_ans:
            ...
            response = openai.Completion.create(
                        ...
                        prompt="### Postgres SQL tables, with their properties:\n\nTable 1:  view_hearings_component(arbitration_id, container_name, matter_number, experts_id, type_of_hearing, start_date, end_date , exclude_weekend, hsf_internal_advocates, external_advocates,hearing_format, virtual_hearing, bundle_used, hsf_involved_bundle, bundle_type, hearing_services_used, experts_who_gave_evidence_at_hearing, created_by, updated_by)\n\nTable 2: view_end_of_matter_assessment_component(arbitration_id ,known_as as container_name ,matter_number ,end_of_matter_id ,file_closed_date ,how_compl x_was_the_matter  ,key_matter_takeway_points ,formal_debrief_client ,client_feedback ,client_responsiveness ,counterparty_difficulty  ,coclaimant_corespondent_difficulty  ,total_hours_recorded_matter_code, total_billable_hours_on_arbitration ,total_revenue_matter ,actual_revenue_reason ,contribution_margin_matter ,total_billed_hours_on_arbitration ,technology_used  ,out_of_house_services  ,created_by ,updated_by)\n\nTable 3: view_settlements_component(arbitration_id, container_name, matter_number, settlements_id, is_adr_process, if_so_which_adr_process, settlement_date, hsf_assessment_success, client_assessment_success, settlement_amount_usd, settlement_amount_local, local_currency_name , further_details, created_by, updated_by)\\n\\n# view_claim_and_counterclaim_component(arbitration_id, known_as as container_name, matter_number, claim_counterclaim_id, value_of_claims_local, local_currency_name, declaratory_relief, value_of_claims_usd, value_of_counterclaims_usd, value_of_counterclaims_local, additional_information_claims  , created_by, updated_by )\n\n Question : A Postgresql query to list all hearing that we did in the last year from 2022, from table view_hearings_component taking column start_date \\nAnswer : SELECT * FROM view_hearings_component WHERE start_date >= '2022-01-01';\n\nQuestion : A Postgresql query to list all top 10 revenue generating matters, from table view_end_of_matter_assessment_component taking column total_revenue_matter - \\nAnswer : SELECT arbitration_id, container_name, matter_number, total_revenue_matter\\nFROM view_end_of_matter_assessment_component WHERE total_revenue_matter IS NOT NULL ORDER BY total_revenue_matter DESC LIMIT 10;\n\npython bar chart visualization code for list all top 10 revenue generating matters:\n\nimport plotly.express as px\nimport pandas as pd\n\n# query the database\ndf = pd.read_sql_query(\"SELECT container_name, total_revenue_matter FROM view_end_of_matter_assessment_component WHERE total_revenue_matter IS NOT NULL ORDER BY total_revenue_matter DESC LIMIT 10;\", conn)\n\n# create the bar chart\nfig = px.bar(df, x='container_name', y='total_revenue_matter', color='container_name')\n\n# update the layout\nfig.update_layout(\n    title='Top 10 containers by total revenue matter',\n    plot_bgcolor='rgba(0,0,0,0)',\n    paper_bgcolor='rgba(0,0,0,0)',\n    font=dict(\n        family=\"Arial\",\n        size=14,\n        color=\"#7f7f7f\"\n    ),\n    showlegend=False\n)\n\n# show the chart\nfig.show()\n\n\nQuestion : A Postgresql query to average settlement values, from table gsettlements_component taking column settlement_amount_usd\n\nQuestion : A Postgresql query to list the arbitrations that have the 10 highest claim values from table view_claim_and_counterclaim_component -\\nAnswer :  SELECT arbitration_id, container_name, matter_number, value_of_claims_usd FROM view_claim_and_counterclaim_component WHERE value_of_claims_usd IS NOT NULL ORDER BY value_of_claims_usd DESC LIMIT 10;\n\nQuestion : A Postgresql query to list all the settlements that we have done in the last year 2022 from table view_settlements_component - \\nAnswer : SELECT settlement_date, container_name FROM view_settlements_component WHERE date_trunc('year', settlement_date) = '2022-01-01';\n\npython bar chart visualization code for list all the settlements that we have done in the last year:\nimport plotly.express as px\nimport pandas as pd\n\n# query the database\ndf = pd.read_sql_query(\"SELECT * FROM view_settlements_component WHERE date_trunc('year', settlement_date) >= (NOW() - INTERVAL '5 years');\", conn)\n\n# create the bar chart\nfig = px.bar(df, x='settlement_date', y='settlement_amount_usd', barmode='group')\n\n# update the layout\nfig.update_layout(\n    title='Settlement Amounts (USD) by Year',\n    xaxis_title='Settlement Date',\n    yaxis_title='Settlement Amount (USD)',\n    xaxis_tickangle=-45,\n    plot_bgcolor='rgba(0,0,0,0)',\n    paper_bgcolor='rgba(0,0,0,0)',\n    font=dict(\n        family=\"Arial\",\n        size=14,\n        color=\"#7f7f7f\"\n    ),\n    showlegend=False\n)\n\nQuestion : Show me all hearing in the past year 5 - \\nAnswer: SELECT arbitration_id, container_name, matter_number, experts_id, type_of_hearing, start_date, end_date, hsf_internal_advocates, external_advocates, hearing_format, virtual_hearing, bundle_used, hsf_involved_bundle, bundle_type, hearing_services_used, experts_who_gave_evidence_at_hearing, created_by, updated_by FROM view_hearings_component\\nWHERE start_date >= (NOW() - INTERVAL '5 years');\n\nFrom the given tables above write a postgresql query:\n\n" + question + "\n\nIdentigy if there are multiple questions in one input, separate them for clarity if there are.\n\n\nStart the answers with 'A no.:' for consistency.\n\nOnly display the answers and no other explanations or random text.\n\n",
                        temperature=0.15,
                        max_tokens=1646,
                        top_p=1,
                        frequency_penalty=0,
